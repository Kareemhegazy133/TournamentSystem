    ...
    public static class TextConnectorProcessor
    {
        ...
        public static List<MatchupEntryModel> ConvertToMatchupEntryModels(this List<string> lines)
        {
            ...
            foreach (string line in lines)
            {
                ...
                MatchupEntryModel mEntry = new MatchupEntryModel();
                mEntry.Id = int.Parse(cols[0]);
                
                if(cols[1].Length == 0)
                {
                    mEntry.TeamCompeting = null;
                }
                else
                {
                    mEntry.TeamCompeting = LookupTeamById(int.Parse(cols[1]));
                }
                
                mEntry.Score = double.Parse(cols[2]);

                int parentId = 0;
                if(int.TryParse(cols[3], out parentId))
                {
                    mEntry.ParentMatchup = LookupMatchupById(parentId);
                }
                else
                {
                    mEntry.ParentMatchup = null;
                }
        ...
        public static void SaveMatchupToFile(this MatchupModel matchup, string matchupFile, string matchupEntryFile)
        {
            ...
            //id = 0, entries=1(pipe delimitedby id), winner=2, matchupRound=3
        ...
        private static string ConvertMatchupEntryListToString(List<MatchupEntryModel> entries)
        {
            ...
            foreach (MatchupEntryModel mEntry in entries)
            {
                output += $"{ mEntry.Id }|";
            }
